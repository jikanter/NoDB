/*======================================================================
 *  WebDev : Web Developer Utilities in JavascriptWebDev
 *======================================================================
 */
 
WebDev.create = function(database) {
    return new WebDev._Impl(database);
};

WebDev.getAttribute = function(elmt, name, splitOn) {
  try { 
    var value = elmt.getAttribute(name);
    if (value == null) { 
        value = elmt.getAttribute("wd:" + name);
    }
    if (splitOn == null) { 
        return value;
    }
    var values = value.split(splitOn);
    for (var i = 0; value = values[i]; i++) { 
      values[i] = value.trim();
    }
    return values;
  } catch (e) { 
      return null;
  }
};

WebDev.getAttributeWithPrefix = function(prefix, elmt, name, splitOn) { 
  try { 
    var value = elmt.getAttribute(prefix + name);
    if (value == null) { 
      value = WebDev.getAttribute(elmt, name, splitOn);
    }
    return value;
  } catch (e) { 
    return null;
  }
};

WebDev.getDojoTypeAttribute = function(elmt) { 
    var dojoType = WebDev.getAttribute(elmt, "dojoType");
    dojoType = dojoType != null ? dojoType : "";
    return dojoType;
};

WebDev.getDataAttribute = function(elmt) { 
    var data = WebDev.getAttribute(elmt, "data");
    data = data != null ? data : "";
    data = data.startsWith("webdev-") ? data.substr("webdev-".length) : role;
    // data = data.startsWith("FreeBase-") ? data.substr("FreeBase-".length) : role;
    return data;
};

WebDev.getRoleAttribute = function(elmt) {
    var role = WebDev.getAttribute(elmt, "role");
    role = role != null ? role : "";
    role = role.startsWith("webdev-") ? role.substr("webdev-".length) : role;
    return role;
};

WebDev.getDisplayAttribute = function(elmt) { 
    var display = WebDev.getAttribute(elmt, "display");
    display = display != null ? display : "";
    display = display.startsWith("display-") ? display.substr("display-".length) : display;
    return display;
};

WebDev.getMashupDataFromDOM = function(elmt) { 
  var c = WebDev.getAttribute(elmt, "data");
  if (c != null && c.length > 0) { 
    try { 
      var o = eval(c);
      if (typeof o == "object") { 
        return o;
      }
    } catch (e) {}
  }
  return {};
};

WebDev.getConfigurationFromDOM = function(elmt) {
    var c = WebDev.getAttribute(elmt, "configuration");
    if (c != null && c.length > 0) {
        try{
            var o = eval(c);
            if (typeof o == "object") {
                return o;
            }
        } catch (e) {}
    }
    return {};
};

WebDev.getExporters = function() {
    Exhibit._initializeExporters();
    return [].concat(Exhibit._exporters);
};

WebDev.addExporter = function(exporter) {
    Exhibit._initializeExporters();
    Exhibit._exporters.push(exporter);
};

/*==================================================
 *  WebDev._Impl
 *==================================================
 */
WebDev._Impl = function(database) {
    this._database = database != null ? 
        database : 
        ("database" in window ?
            window.database :
            WebDev.Database.create());
            
    this._uiContext = Exhibit.UIContext.createRootContext({}, this);
    this._collectionMap = {};
    this._componentMap= {};
    
    this._historyListener = {
        onBeforePerform:        function(action) { if(action.lengthy) { Exhibit.UI.showBusyIndicator();} },
        onAfterPerform:         function(action) { if(action.lengthy) { Exhibit.UI.hideBusyIndicator();} },
        onBeforeUndoSeveral:    function() { Exhibit.UI.showBusyIndicator();},
        onAfterUndoSeveral:     function() { Exhibit.UI.hideBusyIndicator();},
        onBeforeRedoSeveral:    function() { Exhibit.UI.showBusyIndicator();},
        onAfterRedoSeveral:     function() { Exhibit.UI.hideBusyIndicator();}
    };
    SimileAjax.History.addListener(this._historyListener);    
};

WebDev._Impl.prototype.dispose = function() {
    SimileAjax.History.removeListener(this._historyListener);
    
    for (var id in this._componentMap) {
        try{
            this._componentMap[id].dispose();
        }catch(e) {
            SimileAjax.Debug.exception(e, "Failed to dispose component");
        }
    }
    for (var id in this._collectionMap) {
        try{
            this._collectionMap[id].dispose();
        } catch(e) {
            SimileAjax.Debug.exception(e, "Failed to dispose collection");
        }
    }
    
    this._uiContext.dispose();
    
    this._componentMap = null;
    this._collectionMap = null;
    this._uiContext = null;
    this._database = null;
};

WebDev._Impl.prototype.getDatabase = function() {
    return this._database;
};

WebDev._Impl.prototype.getUIContext = function() {
    return this._uiContext;
};

WebDev._Impl.prototype.getCollection = function(id) {
    var collection = this._collectionMap[id];
    if (collection == null && id == "default") {
        collection = Exhibit.Collection.createAllItemsCollection(id, this._database);
        this.setDefaultCollection(collection);
    }
    return collection;
};

WebDev._Impl.prototype.getDefaultCollection = function() {
    return this.getCollection("default");
};

WebDev._Impl.prototype.setCollection = function(id, c) {
    if (id in this._collectionMap) {
        try{
            this._collectionMap[id].dispose();
        } catch(e) {
            SimileAjax.Debug.exception(e);
        }
    }
    this._collectionMap[id] = c;
};

WebDev._Impl.prototype.setDefaultCollection = function(c) {
    this.setCollection("default", c);
};

WebDev._Impl.prototype.getComponent = function(id) {
    return this._componentMap[id];
};

WebDev._Impl.prototype.setComponent = function(id, c) {
    if (id in this._componentMap) {
        try{
            this._componentMap[id].dispose();
        } catch(e) {
            SimileAjax.Debug.exception(e);
        }
    }
    
    this._componentMap[id] = c;
};

WebDev._Impl.prototype.disposeComponent = function(id) {
    if (id in this._componentMap) {
        try{
            this._componentMap[id].dispose();
        } catch(e) {
            SimileAjax.Debug.exception(e);
        }
        delete this._componentMap[id];
    }
};

WebDev._Impl.prototype.getUtility = function(id) { 
    return this._utilityMap[id];
};

WebDev._Impl.prototype.setUtility = function(id, u) { 
    if (id in this._utilityMap) { 
        try { 
            this._utilityMap[id].dispose();
        } catch(e) { 
            SimileAjax.Debug.exception(e);
        }
    }
    
    this._utilityMap[id] = u;
};

WebDev._Impl.prototype.disposeUtility = function(id) { 
    if (id in this._utilityMap) { 
        try{ 
            this._utilityMap[id].dispose();
        } catch(e) { 
            SimileAjax.Debug.exception(e);
        }
        delete this._utilityMap[id];
    }
};



WebDev._Impl.prototype.configure = function(configuration) {
    if ("collections" in configuration) {
        for (var i = 0; i < configuration.collections.length; i++) {
            var config = configuration.collections[i];
            var id = config.id;
            if (id == null || id.length == 0) {
                id = "default";
            }
            this.setCollection(id, Exhibit.Collection.create2(id, config, this._uiContext));
        }
    }
    if ("components" in configuration) {
        for (var i = 0; i < configuration.components.length; i++) {
            var config = configuration.components[i];
            var component = Exhibit.UI.create(config, config.elmt, this._uiContext);
            if (component != null) {
                var id = elmt.id;
                if (id == null || id.length == 0) {
                    id = "component" + Math.floor(Math.random() * 1000000);
                }
                this.setComponent(id, component);
            }
        }
    }
    if ("utilities" in configuration) { 
        for (var i = 0; i < configuration.utilities.length; i++) { 
            var config = configuration.utilities[i];
            var utility = WebDev.UI.create(config, config.elmt, this._uiContext);
            if (utility != null) { 
                var id = elmt.id;
                if (id == null || id.length == 0) { 
                    id = "utility" + Math.floor(Math.random() * 10000000);
                }
                this.setUtility(id, utility);
            }
        }
    }
};

/**
 * Set up this WebDev's view from its DOM configuration.
 * @param {Node} root  optional root node, below which configuration gets read
 *                     (defaults to document.body, when none provided)
 */
WebDev._Impl.prototype.configureFromDOM = function(root) {
    var collectionElmts = [];
    var coderElmts = [];
    var coordinatorElmts = [];
    var lensElmts = [];
    var facetElmts = [];
    var otherElmts = [];
    var f = function(elmt) {
        var role = WebDev.getRoleAttribute(elmt);
        if (role.length > 0) {
            switch (role) {
            case "collection":  collectionElmts.push(elmt); break;
            case "coder":       coderElmts.push(elmt); break;
            case "coordinator": coordinatorElmts.push(elmt); break;
            case "lens":        lensElmts.push(elmt); break;
            case "facet":       facetElmts.push(elmt); break;
            default: 
                otherElmts.push(elmt);
            }
        } else {
            var node = elmt.firstChild;
            while (node != null) {
                if (node.nodeType == 1) {
                    f(node);
                }
                node=node.nextSibling;
            }
        }
    };
    f(root || document.body);
    
    var uiContext = this._uiContext;
    for (var i = 0; i < collectionElmts.length; i++) {
        var elmt = collectionElmts[i];
        var id = elmt.id;
        if (id==null || id.length == 0) {
            id = "default";
        }
        this.setCollection(id, Exhibit.Collection.createFromDOM2(id, elmt, uiContext));
    }
    
    var self = this;
    var processElmts = function(elmts) {
        for (var i = 0; i < elmts.length; i++) {
            var elmt = elmts[i];
            try {
                var component = Exhibit.UI.createFromDOM(elmt, uiContext);
                if (component != null) {
                    var id = elmt.id;
                    if (id == null || id.length == 0) {
                        id = "component" + Math.floor(Math.random() * 1000000);
                    }
                    self.setComponent(id, component);
                }
            } catch (e) {
                SimileAjax.Debug.exception(e);
            }
        }
    };
    processElmts(coordinatorElmts);
    processElmts(coderElmts);
    processElmts(lensElmts);
    processElmts(facetElmts);
    processElmts(otherElmts);
    
    var exporters = WebDev.getAttribute(document.body, "exporters");
    if (exporters != null) {
        exporters = exporters.split(";");
        for (var i = 0; i < exporters.length; i++) {
            var expr = exporters[i];
            var exporter = null;
            
            try {
                exporter = eval(expr);
            } catch (e) {}
            
            if (exporter == null) {
                try { exporter = eval(expr + "Exporter"); } catch (e) {}
            }
            
            if (exporter == null) {
                try { exporter = eval("WebDev." + expr + "Exporter"); } catch (e) {}
            }
            
            if (typeof exporter == "object") {
                Exhibit.addExporter(exporter);
            }
        }
    }
    
    var hash = document.location.hash;
    if (hash.length > 1) {
        var itemID = decodeURIComponent(hash.substr(1));
        if (this._database.containsItem(itemID)) {
            this._showFocusDialogOnItem(itemID);
        }
    }
};

WebDev._Impl.prototype._showFocusDialogOnItem = function(itemID) {
    var dom = SimileAjax.DOM.createDOMFromString(
        "div",
        "<div class='exhibit-focusDialog-viewContainer' id='lensContainer'>" +
        "</div>" +
        "<div class='exhibit-focusDialog-controls'>" +
            "<button id='closeButton'>" + 
                Exhibit.l10n.focusDialogBoxCloseButtonLabel + 
            "</button>" +
        "</div>"
    );
    dom.elmt.className = "exhibit-focusDialog exhibit-ui-protection";
    dom.close = function() {
        document.body.removeChild(dom.elmt);
    };
    dom.layer = SimileAjax.WindowManager.pushLayer(function() { dom.close(); }, false);
    
    var itemLens = this._uiContext.getLensRegistry().createLens(itemID, dom.lensContainer, this._uiContext);
    
    dom.elmt.style.top = (document.body.scrollTop + 100) + "px";
    document.body.appendChild(dom.elmt);
    
    SimileAjax.WindowManager.registerEvent(
        dom.closeButton, 
        "click", 
        function(elmt, evt, target) { SimileAjax.WindowManager.popLayer(dom.layer); },
        dom.layer
    );
};







